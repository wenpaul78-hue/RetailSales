...
import { ChevronLeft, Search, Filter, X, SlidersHorizontal, MoreHorizontal, Disc, Check, ChevronDown, ChevronRight } from 'lucide-react';
import { ReturnOrder, Employee, ReturnOrderStatus } from '../types';

interface ReturnOrderListPageProps {
  onBack: () => void;
  returnOrders: ReturnOrder[];
  onSalesPersonFilterClick: () => void;
  selectedSalesPerson: Employee | null;
  onClearSalesPersonFilter?: () => void;
  onOrderClick: (order: ReturnOrder) => void;
}
...
export const ReturnOrderListPage: React.FC<ReturnOrderListPageProps> = ({ 
  onBack, 
  returnOrders, 
  ...
  const sortOptions: { label: string; value: OrderSortType }[] = [
    { label: '最近退货', value: 'recent' },
    { label: '最久退货', value: 'oldest' },
    { label: '退款最高', value: 'price_high' },
    { label: '退款最低', value: 'price_low' },
  ];
  ...
  const filteredOrders = returnOrders
    .filter(order => {
      ...
      if (searchQuery) {
           match = match && (
              ...
              order.originalOrderId.includes(searchQuery) ||
    ...
    .sort((a, b) => {
      ...
      if (sortBy === 'price_high') return b.amount - a.amount;
      if (sortBy === 'price_low') return a.amount - b.amount;
      return 0;
    });

  const totalRefundAmount = filteredOrders.reduce((sum, order) => sum + (order.amount || 0), 0);
  const orderCount = filteredOrders.length;

  const currentSortLabel = sortOptions.find(opt => opt.value === sortBy)?.label || '最近退货';

  const getStatusBadge = (status: ReturnOrderStatus) => {
    switch(status) {
        case 'pending_audit':
        ...
        case 'pending_refund':
            return <span className="text-[10px] bg-blue-50 text-blue-500 border border-blue-200 px-1.5 py-0.5 rounded">待退款</span>;
        case 'completed':
    ...
    <div className="min-h-screen bg-[#f5f5f5] flex flex-col relative">
       ...
       <div className="bg-[#1a1c29] text-white pb-2 sticky top-0 z-40">
           ...
           <div className="flex items-center justify-between px-4 py-3">
               ...
               <h1 className="text-lg font-medium tracking-wide">销售退货管理</h1>
           ...
           <div className="px-4 mt-2 mb-4">
               <div className="flex justify-between items-center text-xs text-white/70 mb-4">
                   ...
                   <div className="flex items-center gap-4">
                       ...
                       {/* Cancelled More Stats Button */}
                   </div>
               </div>
               <div className="flex items-center justify-around text-center mb-2">
                   <div>
                       <div className="text-xs text-white/70 mb-1">退货金额</div>
                       <div className="text-2xl font-semibold tracking-wider font-mono">¥{totalRefundAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                   </div>
                   <div>
                       <div className="text-xs text-white/70 mb-1">退货单量</div>
       ...
       <div className="bg-white p-3 flex items-center gap-3">
           <div className="flex-1 relative">
               <input 
                   type="text" placeholder="搜索单号/商品名称"
               ...
               filteredOrders.map(order => (
                   <div key={order.id} className="bg-white rounded-lg p-3 shadow-sm mb-3 active:scale-[0.99] transition-transform" onClick={() => onOrderClick(order)}>
                       <div className="flex justify-between items-start mb-3">
                           <div className="flex items-center gap-2">
                               <span className="text-[10px] border border-gray-800 text-gray-800 px-1 py-0.5 rounded-sm">退货</span>
                           ...
                           <div className="flex items-center gap-1.5">
                                ...
                                <div className="w-5 h-5 rounded-full bg-gray-200 overflow-hidden">
                                    {selectedSalesPerson?.avatar ? (
                                        <img src={selectedSalesPerson.avatar} alt="" className="w-full h-full object-cover" />
                                    ) : (
                       ...
                       {order.items.map((item, idx) => (
                        ...
                        <div className="flex justify-between items-center mt-3 pt-2">
                           ...
                           <div className="flex items-center">
                               <span className="text-xs text-gray-500 mr-2">退款:</span>
                               <span className="text-gray-900 font-bold font-mono">
                                   <span className="text-xs">¥</span>{order.amount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                               </span>
                           </div>
                       </div>
                   </div>
